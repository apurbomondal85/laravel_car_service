...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Gallery')) }}</h4>
        </div>
        {!! \App\Library\Html::linkBack(route('admin.gallery.index')) !!}
    </div>

    <form method="post" id="create-form" action="{{ route('admin.gallery.create') }}" enctype="multipart/form-data">
        @csrf
                        ...
                        <div class="form-group row @error('name') error @enderror">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="name"
                                    value="{{ old('name') }}" placeholder="{{ __('Write Project Name') }}"
                                    required>
                                @error('name')
                                    <p class="error-message">{{ $message }}</p>
                                @enderror
