...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Career')) }}</h4>
        </div>
    ...
    <div class="card shadow-sm col-md-8">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.career.create') }}" enctype="multipart/form-data">
                @csrf
                <div class="p-sm-3">
                    ...
                    <div class="form-group row @error('title') error @enderror">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="title"
                                value="{{ old('title') }}" placeholder="{{ __('Write Your Job Title') }}"
                                required>
                            @error('title')
                                <p class="error-message">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="form-group row @error('company_name') error @enderror">
                        <label class="col-sm-3 col-form-label required" for="company_name">{{ __('Company Name') }}</label>
                        <div class="col-sm-9">
                            <textarea type="text" name="company_name" class="form-control" required
                                placeholder="{{ __('Write Short Description') }}" rows="4">{{ old('company_name') }}</textarea>
                            @error('company_name')
                                <p class="error-message">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row @error('deadline') error @enderror">
                        ...
                        <div class="col-sm-9">
                            <input type="date" name="deadline" id="deadline" class="form-control"
                                value="{{old('deadline')}}" min="{{ now()->format('Y-m-d') }}" required>
                            @error('deadline')
                                <p class="error-message">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row @error('short_description') error @enderror">
                        ...
                        <div class="col-sm-9">
                            <textarea type="text" name="short_description" class="form-control"
                                placeholder="{{ __('Write Short Descriptions') }}" rows="4">{{ old('short_description') }}</textarea>
