...
@section('title', 'Thank You Details')
...
<div class="content-wrapper">
    <div class="content-header d-flex justify-content-start">
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Thank You Details')) }}</h4>
        </div>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body py-4">
                    <div class="text-left">
                        ...
                        @if($user_role->hasPermission('thank_you_change_status'))
                            <button class="btn btn-block btn-sm mb-1 {{ $business->is_active ? 'btn-secondary' : 'btn2-secondary' }}" {{ $business->is_payment ? 'disabled' : '' }}
                                onclick="confirmFormModal('{{ route('admin.thanks.change_status.api', $business->id) }}', 'Confirmation', 'Are you sure to change status?')">
                                <i class="fas fa-power-off"></i> {{ $business->is_active ? 'Make Inactive' : 'Make Active' }}
                            </button>
                        @endif

                        @if($user_role->hasPermission('thank_you_update'))
                            <a href="{{ route('admin.thanks.update', $business->id) }}" class="btn btn-block  btn-sm btn-warning mr-1 {{ !$business->is_payment ? '' : 'disabled-link' }}">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                        @endif

                        @if($user_role->hasPermission('thank_you_confirm_payment'))
                            <button class="btn btn-sm btn-block btn-info" {{ !$business->is_payment ? '' : 'disabled' }} {{ $business->is_active ? '' : 'disabled' }}
                                onclick="confirmFormModal('{{ route('admin.thanks.payment.api', $business->id) }}', 'Confirmation', 'Are you sure to confirm payment operation? If once you confirm payment all actions will be disabled this thank you post.')">
                                <i class="fas fa-check"></i> Confirm Payment
                            </button>
                        @endif

                        @if($user_role->hasPermission('thank_you_delete'))
                            <button class="btn btn-sm btn-block btn-danger" {{ $business->is_payment ? 'disabled' : '' }}
                                onclick="confirmFormModal('{{ route('admin.thanks.delete.api', $business->id) }}', 'Confirmation', 'Are you sure to delete operation?')">
