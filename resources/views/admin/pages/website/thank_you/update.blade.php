...
@section('title', __('Update Thank You'))
...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Thank You')) }}</h4>
        </div>
        {!! \App\Library\Html::linkBack(route('admin.thanks.index')) !!}
    </div>

    <div class="card shadow-sm col-md-12">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.thanks.update', [$business->id]) }}" enctype="multipart/form-data">
                @csrf
                <div class="p-sm-3">
                    <div class="row">
                        <div class="col-md-6">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="business_type" required>
                                        ...
                                        @foreach($business_type as $value)
                                            <option value="{{ $value }}" {{(old("business_type", $business->business_type) == $value) ? "selected" : ""}}>
                            ...
                            <div class="form-group row @error('title') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="title"
                                        value="{{ old('title', $business->title) }}" placeholder="{{ __('Write Your Business Title') }}"
                                        required>
                                    @error('title')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="user_id">
                                        ...
                                        @foreach($members as $row)
                                            <option value="{{ $row->id }}" {{(old("user_id", $business->user_id) == $row->id) ? "selected" : ""}}>
                            ...
                            <div class="form-group row @error('contact_person') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="contact_person"
                                        value="{{ old('contact_person', $business->contact_person) }}" placeholder="{{ __('Write Contact Person Details') }}">
                            ...
                            <div class="form-group row @error('description') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <textarea type="text" name="description" class="form-control" required
                                        placeholder="{{ __('Write Description') }}" rows="4">{{ old('description', $business->description) }}</textarea>
                                    @error('description')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group row @error('start_date') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="date" name="start_date" id="start_date" class="form-control"
                                        value="{{old('start_date', $business->start_date)}}" required>
                                    @error('start_date')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row @error('end_date') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="date" name="end_date" id="end_date" class="form-control"
                                        value="{{old('end_date', $business->end_date)}}" required>
                                    @error('end_date')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row @error('amount') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="amount" step="0.01"
                                        value="{{ old('amount', $business->amount) }}" placeholder="{{ __('Write Amount') }}"
                                        required>
                                    @error('amount')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="form-group row @error('note') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <textarea type="text" name="note" class="form-control"
                                        placeholder="{{ __('Write here about payment method & payment details') }}" rows="4">{{ old('note', $business->note) }}</textarea>
                                    @error('note')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row @error('business_logo') error @enderror">
                                <label class="col-sm-3 col-form-label">Bunisess Logo</label>
                                <div class="col-sm-9">
                                    <div class="file-upload-section">
                                        <input name="business_logo" type="file" class="form-control d-none"
                                        ...
                                        <div class="display-input-image"
                                            style="display: {{ $business->business_logo ? '' : 'd-none' }}">
                                            <img src="{{ $business->getBusinessLogo() }}" alt="Preview Image" />
                                            <button type="button"
